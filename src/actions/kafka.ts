...
async function promptForAppName(appPodMap: Map<string, any>, appname: string | undefined | null) {
    ...
    const appNames = Array.from(appPodMap.keys());
...
export async function kafkaConfig(appname: string | undefined | null) {
    ...
    const secretVolumes = pod.spec.volumes.filter((volume: any) => volume.name == "aiven-credentials" ).map((volume: any) => volume.secret.secretName);
