...
async function getValidFileInSource(sourceRepo: string) {
    ...
    const bunFile = Bun.file(path.join(GIT_DIR, sourceRepo, file.file))
    console.log(path.join(GIT_DIR, sourceRepo, file.file))
...
async function copyFileToRepos(
    ...
    const sourceFile = Bun.file(path.join(GIT_DIR, sourceRepo, fileToSync))

    await Promise.all(
        targetRepos.map(async (it) => {
            log(`Copying ${chalk.yellow(`${it.name}/${fileToSync}`)} from ${chalk.yellow(sourceRepo)}`)
            const targetFile = Bun.file(path.join(GIT_DIR, it.name, fileToSync))
            await Bun.write(targetFile, sourceFile)

            const pushResult: PushResult = await createRepoGitClient(it.name).add(fileToSync).commit(message).push()
